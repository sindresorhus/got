...
const shouldProxy = !!process.env.HTTP_PROXY;
const parsedProxy = shouldProxy ? urlParseLax(process.env.HTTP_PROXY) : {};
  ...
  const get = opts => {
    ...
    const req = fn.request(opts, res => {
      ...
      if (isRedirect(statusCode) && 'location' in res.headers && (opts.method === 'GET' || opts.method === 'HEAD')) {
        ...
        const parsedRedirect = urlLib.parse(redirectUrl);
        let redirectOpts = Object.assign({}, opts, parsedRedirect);
        if (shouldProxy) {
          if (parsedRedirect.protocol === 'https:') {
						const connectOptions = { // establishing a tunnel
              host: parsedProxy.hostname,
              port: parsedProxy.port,
              method: 'CONNECT',
              path: `${parsedRedirect.hostname}:443`,
              headers: {
                'Proxy-Authorization': 'Basic ' + new Buffer(parsedProxy.auth).toString('base64'),
              }
            };
            http.request(connectOptions).on('connect', function(res, socket, head) {
              socket.on('error', function() {
                console.log('Socket error', arguments)
              });

              redirectOpts = Object.assign({}, parsedRedirect, {
                socket,
								agent: false,
              });
              ee.emit('redirect');
            ...
            redirectOpts = Object.assign({}, opts, {
              path: redirectUrl,
