...
export class Options {
	...
	_timeout: Delays;
	_prefixUrl: string;
	_body?: string | Buffer | Readable;
	_form?: Record<string, any>;
	_json?: Record<string, any>;
	_url?: URL;
	_cookieJar?: PromiseCookieJar | ToughCookieJar;
	_ignoreInvalidCookies: boolean;
	_searchParameters?: URLSearchParams;
	_dnsCache?: CacheableLookup;
	_context: Record<string, unknown>;
	_hooks: Required<Hooks>;
	_followRedirect: boolean;
	_maxRedirects: number;
	_cache?: string | CacheableRequest.StorageAdapter | false;
	_throwHttpErrors: boolean;
	_username: string;
	_password: string;
	_http2: boolean;
	_allowGetBody: boolean;
	_lookup?: CacheableLookup['lookup'];
	_headers: Record<string, string>;
	_methodRewriting: boolean;
	_dnsLookupIpVersion: DnsLookupIpVersion;
	_parseJson?: ParseJsonFunction;
	_stringifyJson?: StringifyJsonFunction;
	_retry: RequiredRetryOptions;
	_localAddress?: string;
	_socketPath?: string;
	_method: Method;
	_createConnection?: CreateConnectionFunction;
