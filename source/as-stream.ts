...
export default function asStream() {
	...
	proxy.once('options', (options: MergedOptions) => {
		options.retry.retries = () => 0;

		if (options.body) {
			const error = new Error('Got\'s stream is not writable when the `body` option is used');

			if (input.readableLength) {
	...
	proxy._destroy = () => proxy.removeAllListeners('options');
