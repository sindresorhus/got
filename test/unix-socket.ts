...
if (process.platform !== 'win32') {
	test('works', withHttpSocketServer(), async (t, server) => {
	...
	test('protocol-less works', withHttpSocketServer(), async (t, server) => {
		server.get('/', okHandler);
	...
	test('address with : works', withHttpSocketServer(), async (t, server) => {
	...
	test('works when extending instances', withHttpSocketServer(), async (t, server) => {
		server.get('/', okHandler);
	...
	test('passes search params', withHttpSocketServer(), async (t, server) => {
