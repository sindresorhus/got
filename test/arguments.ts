...
test('DNS auto (option translation only)', withServer, async (t, server, got) => {
	server.get('/ok', echoUrl);
	...
	// Even if the request do not use any DNS the ipVersion options should be translated
	t.is((response.request.options as any).family, 0);
});

test('DNS auto', async t => {
	const response = await got('https://api6.ipify.org/', {
		ipVersion: 'auto'
	});

	// The ugliest IPv4 check
	t.true(response.body.includes('.'));
	t.false(response.body.includes(':'));
});

test('DNS IPv4 (option translation only)', withServer, async (t, server, got) => {
	server.get('/ok', echoUrl);
	...
	// Even if the request do not use any DNS the ipVersion options should be translated
	t.is((response.request.options as any).family, 4);
});

test('DNS IPv4', async t => {
	const response = await got('https://api6.ipify.org', {
		ipVersion: 'ipv4'
	});
...
test('DNS IPv6 (option translation only)', withServer, async (t, server, got) => {
	server.get('/ok', echoUrl);
	...
	// Even if the request do not use any DNS the ipVersion options should be translated
	t.is((response.request.options as any).family, 6);
});

test('DNS IPv6', async t => {
	try {
		const response = await got('https://api6.ipify.org', {
			ipVersion: 'ipv6'
		});

		// The ugliest IPv6 check
		t.true(response.body.includes(':'));
	} catch {
